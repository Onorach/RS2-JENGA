...
# Group name matches the SRDF generated by ur_moveit_config: "${prefix}${name}_manipulator"
# With default prefix="" and name="ur" this is "ur_manipulator".
...
def build_motion_plan_request(
    ...
    if start_state is not None:
        ...
        # is_diff=True with an empty RobotState means "use move_group's current known
        # state as the start".  Without this, MoveIt receives empty joint positions and
        # logs "Found empty JointState message", which can cause spurious collision checks
...
class MoveItPlanningInterface(Node):
    ...
    def wait_for_server(self, timeout_sec: float = 30.0) -> bool:
        ...
        self.get_logger().info(
            f"Waiting for MoveGroup action '{self._move_action_name}' (timeout {timeout_sec:.1f}s)..."
        )
    ...
    def plan_to_pose(
        ...
        self.get_logger().info(f"Sending plan request to MoveGroup (plan_only={plan_only}).")
        ...
        if res.error_code.val != 1:
            self.get_logger().warn(
                f"Planning failed with error_code={res.error_code.val}."
            )
            return None
        ...
        self.get_logger().info(
            f"Planning succeeded ({len(res.planned_trajectory.joint_trajectory.points)} points)."
        )
    ...
    def add_floor_plane(
        ...
        self.get_logger().info(
            f"Floor-plane exclusion zone added: top at z={floor_z:.3f} in '{frame_id}'."
        )
