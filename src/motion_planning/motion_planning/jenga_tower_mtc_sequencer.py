...
def parametric_platform_offset(data: dict[str, Any]) -> tuple[float, float]:
    """XY shift (metres) applied to all parametric stock picks and tower places."""
...
def _stock_pick_xyz_list(
    ...
    *,
    ...
    if "rows" in stock:
        ...
        ox, oy = xy_offset
        return [(x + ox, y + oy, z) for x, y, z in trimmed]
...
def _parametric_steps(data: dict[str, Any]) -> list[tuple[Pose, Pose]]:
    ...
    pick_xyz = _stock_pick_xyz_list(g, n_tower=n, xy_offset=parametric_platform_offset(data))
    if len(pick_xyz) < n:
        raise ValueError(
