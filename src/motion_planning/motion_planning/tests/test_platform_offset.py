...
def test_parametric_platform_offset_defaults_to_zero():
    data = _load_layout()
    assert parametric_platform_offset(data) == (0.0, 0.0, 0.0)
...
def test_platform_offset_shifts_tower_and_stock_equally():
    ...
    shifted["parametric"]["platform_offset"] = {"x": 0.01, "y": -0.02, "z": 0.03}

    ox, oy, oz = 0.01, -0.02, 0.03
    p = base["parametric"]
    t = p["tower"]
    ...
    assert tower_shift.z == pytest.approx(tower_base.z + oz)

    stock_base = _stock_pick_xyz_list(p["stock"], n_tower=n, xyz_offset=(0.0, 0.0, 0.0))[0]
    stock_shift = _stock_pick_xyz_list(
        shifted["parametric"]["stock"],
        n_tower=n,
        xyz_offset=parametric_platform_offset(shifted),
    )[0]
    assert stock_shift[0] == pytest.approx(stock_base[0] + ox)
    assert stock_shift[1] == pytest.approx(stock_base[1] + oy)
    assert stock_shift[2] == pytest.approx(stock_base[2] + oz)
