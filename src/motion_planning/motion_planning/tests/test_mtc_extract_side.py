...
def main(args=None) -> int:
    ...
    node = Node("test_mtc_extract_side")
    action_name = str(node.declare_parameter("action_name", "jenga_extract_side_block").value)
    goal_frame = str(node.declare_parameter("goal_frame", "world").value)

    client = ActionClient(node, JengaExtractSideBlock, action_name)
    ...
    block.pose = Pose(
        position=Point(x=-0.018, y=0.315, z=0.059),
    ...
    place.pose = Pose(
        position=Point(x=-0.2125, y=0.30, z=0.0141),
        orientation=Quaternion(x=0.0, y=0.0, z=0.0, w=1.0),
    )

    goal = JengaExtractSideBlock.Goal()
    goal.block_index = 12
    goal.block_pose = block
    goal.place_pose = place

    node.get_logger().info("Sending MTC extract-side action...")
